...
Author this Launchable in the Brev web Console only. The setup script copies `NVIDIA_API_KEY` from the short-lived setup environment to `${HOME}/.config/nvmolkit/NVIDIA_API_KEY`, outside the repository, with directory mode `0700` and file mode `0600`. The notebook loads that file automatically and does not request the key. The key remains on the VM disk until the environment is deleted or the file is removed. Never put API keys in this file, the repository, notebook outputs, logs, screenshots, or chat.
