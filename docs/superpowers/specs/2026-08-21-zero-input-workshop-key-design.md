...
The repository will contain a secret-free setup template and a trusted offline
renderer. The Brev Console will hold the separately rendered setup-script body
with the approved workshop key. The user has already updated that saved Console
body and removed `NVIDIA_INFERENCE_API_KEY` from Setup values.
...
4. Keep the repository setup source useful as a redacted operator template that
   is rendered only through the trusted offline renderer.
...
Use `launchable/render_setup.py` to create the private Console copy. From the
repository, the operator runs:

```bash
python3 launchable/render_setup.py /private/tmp/nvmolkit-workshop-setup.sh
```

The renderer reads the workshop key only at a hidden prompt. It rejects empty,
multiline, NUL-containing, and non-`sk-` values; shell-quotes the accepted value;
requires exactly one sentinel; refuses repository, existing, and unsafe output
paths; and creates the output outside the repository with mode `0600`. Manual
sentinel replacement is not an approved workflow.

The repository template must fail before dependency installation when the
sentinel has not been rendered. A rendered Console copy must:
...
The operator pastes only the rendered file's contents into the saved Brev
Console setup body, never commits, uploads, attaches, or shares the rendered
file, and deletes the private rendered file after saving the Console body. A
repository push does not replace the body already saved in the Console.

## Attendee experience

The Launchable has **No Launch parameters or Setup values**. The workshop
organizer provisions the approved Inference Hub credential in the saved Console
setup body. Attendees do not see a credential field and do not need an NVIDIA
API account or API key.
...
4. Render a temporary test copy with a fake key through
   `launchable/render_setup.py` and verify renderer validation, shell quoting,
   safe outside-repository output, mode `0600`, atomic persistence, no output
   leakage, and no reliance on credential environment variables.
